    ...
    public class RepoRegistros : IRepoRegistros
    {
        ...
        public bool Alta(Registro obj)
        {
            ...
            if (obj != null)
            {
                ...
                string sql1 = "insert into INGRESOACT(CodigoAct, cedula, fecha) values(@CodigoAct, @cedula, @fecha);";
        ...
        public List<Registro> FiltrarIngresos(string cedula, DateTime fechaInicio, DateTime fechaFinal)
        {
            ...
            string sql = "select CodigoAct from INGRESOACT where cedula=@cedula AND DAY(fecha) <= @fechaFinal AND DAY(fecha) >= @fechaInicio";
            SqlCommand com = new SqlCommand(sql, con);
            com.Parameters.AddWithValue("@cedula", cedula);
            com.Parameters.AddWithValue("@fechaInicio", fechaInicio.Day);
            com.Parameters.AddWithValue("@fechaFinal", fechaFinal.Day);
            try
            {
                con.Open();
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        ...
                        {
                            CodigoAct = 20/*reader.GetInt32(0)*/,
                            Cedula = cedula,
                        });
                    }
                }
